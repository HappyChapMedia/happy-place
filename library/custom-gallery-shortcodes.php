<?php
/**
 * Creates a slider for galleries
 */
...
add_shortcode('slick_slider', function($atts)
{
    ...
    foreach( array_filter( array_map(function($id){ return (int) $id; }, explode(',', $ids)) ) as $media_id)
    {
        ...
        {
            $html_output .= '<div><div class="image"><a class="lightbox-link" href="'.esc_url($image_data[0]).'" data-lightbox="gallery-1"><img src="'.esc_url($image_data[0]).'" height="'.$image_data[0].'" width="'.$image_data[0].'" /></a></div></div>';
        }
    }
...
add_shortcode('gallery', function($atts)
{
    ...
    foreach( array_filter( array_map(function($id){ return (int) $id; }, explode(',', $ids)) ) as $media_id)
    {
        ...
        if( $image_data = wp_get_attachment_image_src( $media_id, $size ) )
        {
            $full_image = wp_get_attachment_image_src($media_id, 'full' );
            $html_output .= '<div><div class="image"><a class="lightbox-link" href="'.esc_url($full_image[0]).'" data-lightbox="lightbox-'. $attrs['slider_class_name'].'"><img src="'.esc_url($image_data[0]).'" height="'.$image_data[0].'" width="'.$image_data[0].'" /></a></div></div>';
        }
    }
